-- =============================================================
-- LESSON TRACKER PRO - COMPLETE WORKING SCHEMA
-- Columns match BOTH the form screens AND the sync service
-- Run ONCE in Supabase SQL Editor
-- =============================================================

-- =============================================================
-- PART 1: GRANTS (must be first, before tables)
-- =============================================================
...
-- 3.1 PROFILES
-- Note: id is NOT FK to auth.users (instructor creates pupils before they sign up)
...
-- 3.4 PUPILS
-- Has ALL columns from both form screen direct insert AND sync service
-- id is FK to profiles (pupil is also a profile)
CREATE TABLE pupils (
...
-- 3.7 PUPIL INVITE LINKS & SUBMISSIONS
...
-- 3.10 CALENDAR EVENTS (instructor diary)
...
-- 3.11 EVENTS (admin announcements)
CREATE TABLE events (
...
-- 3.12 TRANSACTIONS (income & expense records)
...
-- 3.28 PAYMENT REQUESTS (instructor → admin)
...
-- 3.29 INSTRUCTOR PAYMENTS (admin payments to instructors)
...
-- 3.30 PAYMENTS (pupil payments to instructors)
...
-- 3.33 APP CONTENT (privacy policy, terms)
...
-- PROFILES
CREATE POLICY "profiles_all_insert" ON profiles FOR INSERT WITH CHECK (true);
...
-- INVITE SUBMISSIONS (public write for signup flow)
CREATE POLICY "invite_submissions_insert" ON pupil_invite_submissions FOR INSERT WITH CHECK (true);
CREATE POLICY "invite_submissions_admin" ON pupil_invite_submissions FOR ALL USING (public.is_admin());
...
CREATE POLICY "slots_pupil_read" ON open_slots FOR SELECT USING (is_booked = false);
...
-- MESSAGES
CREATE POLICY "messages_participant_all" ON messages FOR ALL USING (auth.uid() IN (sender_id, receiver_id));

-- NOTIFICATIONS
CREATE POLICY "notifications_user_all" ON app_notifications FOR ALL USING (user_id = auth.uid());
...
CREATE POLICY "resources_public_or_selective" ON teaching_resources FOR SELECT USING (visibility IN ('public', 'selective') OR instructor_id = auth.uid());
...
-- INSTRUCTOR PAYMENTS
CREATE POLICY "instructor_payments_instructor_read" ON instructor_payments FOR SELECT USING (instructor_id = auth.uid());
