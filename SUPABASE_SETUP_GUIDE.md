...
> The definitive schema (`SUPABASE_SCHEMA.sql`) is the single source of truth. It includes everything: all tables, all enums, indexes, triggers, RLS policies (with `is_admin()` security definer function to avoid recursion), seed data, and the auth trigger that auto-creates a profile row on user signup.
> 
> **NOTE:** If you have any other SQL files, they are obsolete. Only use `SUPABASE_SCHEMA.sql`.
